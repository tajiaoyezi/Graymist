...
- **永不回显**:响应只暴露 `has_api_key: true/false`,**MUST NOT** 返回明文或密文;UI 仅以「已配置」徽标标识(未配置不标)。
- **422 不泄漏**:自定义 `RequestValidationError` 处理器,422 响应只回 `type/loc/msg`、剥离 `input`/`ctx`,杜绝提交体里的明文 key 经校验错误回显(默认 FastAPI 处理器会把整个提交体塞进 `input`)。
...
- `inference-api`: 南向密钥来源从「仅 `auth_ref` 引用持有」扩展为「版本加密存储凭证(解密)> `auth_ref` 环境变量 > 无」的优先级解析(同步修订「external-api 来源真实推理」与「canonical 内核与南向协议适配器」两条要求的措辞)。

## Impact

- **后端新增**:`app/common/crypto.py`(Fernet 加解密 + 主密钥校验)、迁移 `0006_auth_secret.py`。
- **后端修改**:`db/tables.py`(`ModelVersionRow.auth_secret_enc` 列 + `has_api_key` 属性)、`config.py`(`secret_key`)、`versions/schemas.py`(`api_key` 写入字段 / `has_api_key` 出参)、`versions/service.py`(创建/轮换时加密)、`versions/router.py`(`PUT /versions/{id}/credential`)、`external.py`(解密优先注入)、`main.py`(`RequestValidationError`→422 脱敏 + `SecretKeyNotConfiguredError`→400)、`pyproject.toml`(加 `cryptography`)。
- **前端修改**:`components/NewVersionForm.tsx`(API Key 输入)、`types.ts`、`i18n/locales/zh.ts`、模型详情版本列表「已配置密钥」徽标。
