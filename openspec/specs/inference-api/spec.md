...
平台的推理调用契约 —— 同步/异步推理、推理输入 Schema 校验、A/B 权重路由与实际命中记录、每端点并发限流(同步 429 / 异步排队)、推理日志。对应原始需求 §4.3 / 2.3。由 `a3-inference-api` 引入。v1.0 推理为**模拟**执行(`sleep` 计时 + 按命中 Model 的 `output_schema` 生成占位结果),不调用任何外部模型后端,严守 v1.0 范围(不含 external-api 来源、流式 SSE、成本/用量计量、token/LLM 指标 —— 均归 v1.1+)。仅 `running` 端点可推理。
