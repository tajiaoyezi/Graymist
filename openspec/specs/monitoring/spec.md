...
平台的监控指标聚合与查询契约 —— 按端点 + 时间窗(1h 按分钟 / 24h 按小时 / 7d 按天)聚合 QPS、平均延迟、P99 延迟、错误率,按实际命中版本分组(A/B 对比),当前并发数实时 gauge,以及资源总览(复用平台配额)。对应原始需求 §4.4 / 2.4。由 `a4-monitoring` 引入。指标从 `inference_log` **按需聚合**(不建指标表);延迟样本仅取实际执行的调用(success/timeout),error/rate_limited 只计错误率分母。严守 v1.0 范围:不含成本/用量计量、token/LLM 专属指标、告警与通知、分布式追踪(均归 v1.1+)。
