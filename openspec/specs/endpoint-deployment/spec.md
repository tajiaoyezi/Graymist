...
推理端点的部署与流量管理 —— 把 `ready` 版本部署为可调用的推理端点(Endpoint),含端点实体与唯一 URL 路径、独立的端点状态机(creating/running/stopped/failed,failed 可经重启恢复)、A/B 版本绑定与权重原子一致性、异步部署/更新/停止/重启(API 立即返回 + 后台执行 + 客户端轮询)、平台资源配额三维累计校验,以及部署管控台前端页面(端点列表/创建编辑/权重与配额配置/状态实时刷新/启停重启)。对应原始需求 2.2(推理端点部署,后端核心难点)+ 2.6(部署管控台),属 v1.0 核心能力。建立在 model-registry 的"仅 ready 可部署"契约之上;`timeout_ms`/`max_concurrency` 本能力仅存储,其超时与限流执行属推理调用 API。
